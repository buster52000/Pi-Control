...
public class GSpeechDuplex {
	...
	public static void main(String args[]) {
		 GSpeechDuplex dup = new
		 GSpeechDuplex("AIzaSyBOti4mM-6x9WDnZIjIeyEU21OpBXqWBgw");
		 dup.addResponseListener(new GSpeechResponseListener() {
		
		 @Override
		 public void onResponse(GoogleResponse gr) {
		 System.out.println(gr.getAllPossibleResponses().toString());
		 }
		 });
		
		 AudioFormat af = Microphone.getAudioFormat();
		 Microphone mic = new Microphone(FLACFileWriter.FLAC);
		 try {
			dup.recognize(mic.getTargetDataLine(), af);
	 ...
	 * 
	 * @param API_KEY
	 *            The API-Key for Google's Speech API. An API key can be
	 *            obtained by requesting one by following the process shown at
	 *            this <a
	 *            href="http://www.chromium.org/developers/how-tos/api-keys"
	 *            >url</a>.
	 */
	...
	public void recognize(byte[] data, int sampleRate) {
		...
		if (data.length >= MAX_SIZE) {// Temporary Chunking. Does not allow for
										// Google to gather context.
	...
	public void recognize(TargetDataLine tl, AudioFormat af) throws IOException, LineUnavailableException {
		...
		// Generates the Downstream URL
		final String API_DOWN_URL = GOOGLE_DUPLEX_SPEECH_BASE + "down?maxresults=5&pair=" + PAIR;

		// Generates the Upstream URL
		final String API_UP_URL = GOOGLE_DUPLEX_SPEECH_BASE + "up?lang=" + language + "&lm=dictation&client=chromium&pair=" + PAIR + "&key=" + API_KEY + "&pushback&continuous&&interim"; // Tells
																																										// Google
																																										// to
																																										// constantly
																																										// monitor
		...
		// TODO Add implementation that sends feedback in real time. Protocol
	...
	private void downChannel(String urlStr) {
		...
		new Thread("Downstream Thread") {
			public void run() {
				...
				if (inStream == null) {
					// ERROR HAS OCCURED
					System.out.println("failed instream connection: " + url);
				}
				while (inStream.hasNextLine()) {
					String response = inStream.nextLine();
					System.out.println("Nextline");
					System.out.println(response);
					if (response.length() > 17) {// Prevents blank responses
													// from Firing
	 ...
	 * @param mtl
	 *            The TargetDataLine you want to post data from. <b>Note should
	 *            be open</b>
	 * @param maf
	 *            The AudioFormat of the data you want to post
	 */
	private void openHttpsPostConnection(final String murl, final TargetDataLine mtl, final AudioFormat maf) {
		URL url;
		try {
			url = new URL(murl);
			URLConnection urlConn = url.openConnection();
		...
		try {
			...
			try {
				...
				for (byte[] dataArray : mextrad) {
					out.write(dataArray); // one big block supplied instantly to
											// the underlying chunker wont work
											// for duration > 15 s.
					try {
						Thread.sleep(1000);// Delays the Audio so Google thinks
											// its a mic.
	 ...
	 * 
	 * @param The
	 *            File you want to get the byte[] from.
	 * @return The byte[]
	 * @throws IOException
	...
	private void parseResponse(String rawResponse, GoogleResponse gr) {
		...
		if (array.contains("\"confidence\":")) {// Removes confidence phrase if
												// it exists.
