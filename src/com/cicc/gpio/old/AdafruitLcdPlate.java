...
public class AdafruitLcdPlate implements Lcd {
	...
	static final int[] SHIFT_REVERSE = { 0x00, 0x10, 0x08, 0x18, 0x04, 0x14, 0x0C, 0x1C, 0x02, 0x12, 0x0A, 0x1A, 0x06, 0x16, 0x0E, 0x1E };
	...
	// button directions
	public static final int LCD_BUTTON_SELECT = 0;
	public static final int LCD_BUTTON_RIGHT = 1;
	public static final int LCD_BUTTON_DOWN = 2;
	public static final int LCD_BUTTON_UP = 3;
	public static final int LCD_BUTTON_LEFT = 4;
	...
	private int[] cMapIDs = new int[8];

	public AdafruitLcdPlate() throws IOException, InterruptedException {

		this.columns = 16;
		this.rows = 2;
		for (int i = 0; i < cMapIDs.length; i++)
			cMapIDs[i] = -1;

		I2CBus i2cBus = I2CFactory.getInstance(I2CBus.BUS_1);
		device = i2cBus.getDevice(0x20);
		...
		byte[] piPlateSetupRegisters = { 0x3F, // IODIRA R+G LEDs=outputs,
												// buttons=inputs
	...
	public void createChar(byte location, byte[] charmap, int charID) throws IOException {
		int x = cPosX;
		int y = cPosY;
		...
		writeCmd(0x28); // 2 line 5x8 matrix
		writeCmd(LCD_CURSORSHIFT | 0);
		writeCmd(LCD_DISPLAYCONTROL | displayControl);
		setCursorPosition(y, x);
		cMapIDs[location] = charID;
	}

	public int getCharIDAt(int location) {
		if (location >= 0 && location < cMapIDs.length)
			return cMapIDs[location];
		else
			return -1;
	}
	...
	protected void pollWait() throws IOException {
		...
		if ((ioDirB & 0x10) != 0) {
			...
			while (true) {
				...
				device.write(MCP23017_GPIOB, new byte[] { (byte) lo, (byte) hi, (byte) lo }, 0, 3); // Strobe
																									// low,
																									// high,
																									// low.
																									// Second
	...
	public int buttonsPressedBitmask() throws IOException {
		return device.read(MCP23017_GPIOA) & 0x1F;
	}
